...
export default function Cars({ layout }: { layout: SceneLayout }) {
  ...
  const anims = useMemo<CarAnim[]>(
    () => lanes.map(() => ({ heat: 0, jumpY: 0, jumpVel: 0, airborne: false, squashT: 0, exploded: 0 })),
    [lanes],
  );
  ...
  const bump = (i: number, x: number, y: number, z: number, pan: number) => {
    const a = anims[i];
